...
class Evenement
{
    ...
    /** Libellé retourné par le géocodeur (OpenStreetMap), pour l’info-bulle sur la carte. */
    #[ORM\Column(name: 'location_display_name', length: 500, nullable: true)]
    ...
    /** Pourcentage de réduction appliqué au prix de base (1–100). Null = pas de promo. */
    #[ORM\Column(name: 'promo_percent', type: Types::SMALLINT, nullable: true)]
    #[Assert\Range(min: 1, max: 100, notInRangeMessage: 'La promotion doit être entre {{ min }} et {{ max }} %')]
    private ?int $promoPercent = null;
    ...
    public function setLocationDisplayName(?string $locationDisplayName): static
    {
    ...
    {
        if ($promoPercent === null || $promoPercent < 1) {
            $this->promoPercent = null;
        } else {
            $this->promoPercent = min(100, $promoPercent);
        }
    ...
    {
        return $this->promoPercent !== null && $this->promoPercent > 0;
    }

    /** Prix par place après application de la promo (le champ prix reste le prix de base). */
    public function getPrixFinal(): float
    {
        $base = (float) ($this->prix ?? 0);
        $pct = (float) ($this->promoPercent ?? 0);
