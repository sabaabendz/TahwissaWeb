...
class GithubAuthenticator extends OAuth2Authenticator
{
    ...
    {
        $client = $this->clientRegistry->getClient('github_client');
        ...
        return new SelfValidatingPassport(new UserBadge($accessToken->getToken(), function () use ($client, $accessToken): User {
            ...
            if ($email === '') {
                throw new CustomUserMessageAuthenticationException('GitHub did not return an email address.');
            }
            ...
            if (!$user instanceof User) {
                ...
                $user->setName((string) ($githubUser->getName() ?? $githubUser->getNickname() ?? $email));
