...
class ReservationTransportController extends AbstractController
{
    ...
    public function index(Request $request, ReservationTransportRepository $repo, \App\Repository\UserRepository $userRepo, PaginatorInterface $paginator): Response
    {
        ...
        $query = $repo->createQueryBuilder('r')->orderBy('r.dateReservation', 'DESC')->getQuery();
        $reservations = $paginator->paginate($query, $request->query->getInt('page', 1), 8);
        
        // Get all reservations for statistics (not paginated)
    ...
    public function edit(Request $request, ReservationTransport $reservationTransport, EntityManagerInterface $em, EmailService $emailService, \App\Repository\UserRepository $userRepo): Response
    {
        $this->checkRole($request);
        
        $oldStatut = $reservationTransport->getStatut();
        
        $form = $this->createForm(ReservationTransportType::class, $reservationTransport, ['user_role' => 'ADMIN', 'is_edit' => true]);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            $em->flush();
            
            // Send confirmation email if status changed to CONFIRMEE
            if ($oldStatut !== 'CONFIRMEE' && $reservationTransport->getStatut() === 'CONFIRMEE') {
                $emailService->sendReservationConfirmation($reservationTransport);
                $this->addFlash('success', 'Réservation modifiée avec succès et email de confirmation envoyé !');
            } else {
        ...
        // Get user name
        $user = $userRepo->find($reservationTransport->getIdUser());
        $userName = $user ? $user->getName() : 'Utilisateur Inconnu';
