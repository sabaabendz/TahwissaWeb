...
class AdminController extends AbstractController
{
    ...
    public function voyageShow(Request $request, Voyage $voyage, UserRepository $userRepository): Response
    {
        $this->checkRole($request, 'ADMIN');
    ...
    #[Route('/voyage/{id}/delete', name: 'admin_voyage_delete', requirements: ['id' => '\d+'])]
    public function voyageDelete(Request $request, Voyage $voyage, EntityManagerInterface $em): Response
    {
        ...
        $reservationCount = $em->getRepository(\App\Entity\ReservationVoyage::class)
            ->count(['voyage' => $voyage]);

        if ($reservationCount > 0) {
            $this->addFlash('danger', "Impossible de supprimer ce voyage : {$reservationCount} réservation(s) y sont liées. Annulez ou supprimez d'abord les réservations.");
            return $this->redirectToRoute('admin_voyage_show', ['id' => $voyage->getId()]);
        }
    ...
    public function reservationDelete(Request $request, ReservationVoyage $reservation, EntityManagerInterface $em): Response
    {
        ...
        if ($this->isCsrfTokenValid('delete' . $reservation->getId(), $request->request->get('_token'))) {
            // Remettre les places en stock si la réservation n'était pas déjà annulée
    ...
    #[Route('/voyage/insights', name: 'admin_voyage_insights', methods: ['POST'])]
    public function voyageInsights(Request $request, VoyageInsightsService $insightsService): JsonResponse
    {
        ...
        if (!$insightsService->isEnabled()) {
            return new JsonResponse(['error' => 'Service IA non configur\u00e9 (cl\u00e9 API Gemini manquante).'], 503);
        }
    ...
    public function reservationPdf(Request $request, ReservationVoyage $reservation, UserRepository $userRepository): Response
    {
        $this->checkRole($request, 'ADMIN');
