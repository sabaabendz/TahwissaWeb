...
public function voyageReact(
    ...
    public function chatbotSend(Request $request, TravelChatbotService $chatbotService): JsonResponse
    {
        ...
        if (!$this->isCsrfTokenValid('client_chatbot', $csrfToken)) {
            return new JsonResponse(['success' => false, 'reply' => 'Session invalide. Rechargez la page et réessayez.'], 419);
        }

        $data = json_decode($request->getContent(), true);
        if (!is_array($data)) {
            return new JsonResponse(['success' => false, 'reply' => 'Requête invalide.'], 400);
        }
